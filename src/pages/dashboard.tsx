...
import { getVotingResults } from 'apiSdk/voting-results';
import { VotingResultInterface } from 'interfaces/voting-result';

import { VoteListPage as ShowCaseList } from 'pages/votes';

function DashboardPage() {
  const { session } = useSession();

  const voteStat = useSWR<PaginatedInterface<VoteInterface>>(
    () => `/votes`,
    () => getVotes({ limit: 1, offset: 0 }),
  );

  const votingResultStat = useSWR<PaginatedInterface<VotingResultInterface>>(
    () => `/voting-results`,
    () => getVotingResults({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Votes',
      result: voteStat?.data?.totalCount,
      isLoading: voteStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Voting Results',
      result: votingResultStat?.data?.totalCount,
      isLoading: votingResultStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`votes`} passHref legacyBehavior>
                  <Button
