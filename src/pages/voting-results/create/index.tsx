...
import { createVotingResult } from 'apiSdk/voting-results';
...
function VotingResultCreatePage() {
  ...
  const handleSubmit = async (values: VotingResultInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createVotingResult(values);
      resetForm();
  ...
  const formik = useFormik<VotingResultInterface>({
    initialValues: {
      selected_option: '',
      vote_id: (router.query.vote_id as string) ?? null,
