...
import { getVotingResultById } from 'apiSdk/voting-results';
import { VotingResultInterface } from 'interfaces/voting-result';

function VotingResultViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('voting_result', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/voting-results/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Selected Option" text={data?.selected_option} />
                ...
                {hasAccess('vote', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Vote"
                    text={
                      <Link as={NextLink} href={`/votes/view/${data?.vote?.id}`}>
                        {data?.vote?.title}
                      </Link>
                    }
                  />
